...
fn most_income(order_list: &mut Vec<Order>) -> usize {
    ...
    let mut cache = vec![vec![0usize; order_list.len()+1]; max_end_time + 1];

    for i in 0..=max_end_time {

        for j in 0..=order_list.len() {
            if i == 0 || j==0 {
                cache[i][j] = 0;
            } else {

                let order = &order_list[j-1];
                //use
                let order_price = order.price;
                let order_end_time = order.start + order.duration;
                if i>= order_end_time{
                    let pre_end_time = order.start;
                    let pre_max_income = cache[pre_end_time][j-1];
                    let use_max_income = pre_max_income + order.price;

                    let not_use_max_income = cache[i][j-1];

                    cache[i][j] = if use_max_income > not_use_max_income{
                        use_max_income
                    }else if use_max_income<not_use_max_income{
                        not_use_max_income
                    }else{
                        use_max_income
                    };
                    ...
                    let not_use_max_income = cache[i][j-1];
                    cache[i][j] = not_use_max_income;
                }
            }
        }
    }

    cache[max_end_time][order_list.len()]
}
