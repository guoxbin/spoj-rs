...
fn main() {
    ...
    for _i in 0..len {
        buf.clear();
        read_from_stdin(&mut buf).unwrap();
        let a = buf.trim().to_owned();

        buf.clear();
        read_from_stdin(&mut buf).unwrap();
        let b = buf.trim().to_owned();
...
fn process(a: &str, b: &str) {
    let r = find_common(a.as_bytes(), 0, b.as_bytes(), 0);
    let mut r = collect(r);
    sort(&mut r);
    let r = uniq(r);
    for i in r {
...
fn find_common(a: &[u8], ai: usize, b: &[u8], bi: usize) -> Vec<Vec<u8>> {
    ...
    for i in ai..a.len() {
        for j in bi..b.len() {
            if a[i] == b[j] {
                let sub_result = find_common(a, i + 1, b, j + 1);
